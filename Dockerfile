# mcp-hosting — nginx (static public) + Filebrowser (/files/) + MCP (/mcp)
ARG FB_VERSION=2.60.0

FROM node:20-alpine AS builder-mcp
WORKDIR /opt/mcp-hosting/mcp-server
COPY mcp-server/package.json ./
RUN npm install --omit=dev --no-audit --no-fund
COPY mcp-server/ ./

FROM filebrowser/filebrowser:v${FB_VERSION} AS builder-filebrowser

FROM nginx:1.27-alpine

RUN apk add --no-cache nodejs ca-certificates \
  && mkdir -p /var/www/public /var/lib/filebrowser /opt/mcp-hosting

COPY --from=builder-filebrowser /filebrowser /usr/local/bin/filebrowser
COPY --from=builder-mcp /opt/mcp-hosting/mcp-server /opt/mcp-hosting/mcp-server
COPY nginx/default.conf /etc/nginx/conf.d/default.conf
COPY docker/entrypoint.sh /entrypoint.sh
COPY public/index.html /opt/mcp-hosting/seed/index.html
COPY VERSION /opt/mcp-hosting/VERSION

RUN chmod +x /entrypoint.sh /usr/local/bin/filebrowser

ENV PUBLIC_ROOT=/var/www/public \
    MCPHOSTING_PUBLIC_ROOT=/var/www/public \
    MCPHOSTING_MCP_LISTEN=127.0.0.1:3101

VOLUME ["/var/www/public", "/var/lib/filebrowser"]
EXPOSE 80

ENTRYPOINT ["/entrypoint.sh"]
